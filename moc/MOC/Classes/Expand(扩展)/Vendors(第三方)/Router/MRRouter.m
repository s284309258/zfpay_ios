...
@property (nonatomic, strong) NSMutableArray<NSString *> *defaultSubClasses;
...
- (_MRRoute *) routeWithURL:(NSString *)URLPattern {
    ...
    if (!route) {
        ...
        [[MRRouter sharedInstance].defaultSubClasses addObject:route.className];
...
- (NSObject *)executeDefaultBlock:(_MRRoute *)route prepareBlock:(MRPrepareBlock)prepareBlock completeBlock:(MRCompleteBlock)completeBlock {
    ...
    if (self.defaultExecutingBlock) {
        self.defaultExecutingBlock(object ?:route.host, route.parameters);
    }
    if (completeBlock) {
...
//    _defaultSubClasses = [defaultClassType subClasses];
    _defaultSubClasses = [[NSMutableArray alloc] init];
}
