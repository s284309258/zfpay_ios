...
#define kDefaultBadgeBackgroundColor RGB(255.0, 59.0, 48.0)
#define kDefaultOverlayColor [UIColor colorWithWhite:1.0f alpha:0.3]

#define kDefaultBadgeTextFont [UIFont systemFontOfSize:12]
...
//新加的方法
- (id)initWithParentView:(UIView *)parentView alignment:(JSBadgeViewAlignment)alignment frame:(CGRect)rect
{
...
- (void)layoutSubviews
{
    ...
    CGFloat viewWidth =  self.isCircle?20: textWidth + kBadgeTextSideMargin + (kMarginToDrawInside * 2);
    CGFloat viewHeight = self.isCircle?20: kBadgeHeight + (kMarginToDrawInside * 2);
    ...
    switch (self.badgeAlignment) {
        ...
        case JSBadgeViewAlignmentCenter:
            ...
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        case JSBadgeViewAlignmentInnerTopRight:
...
{
    //根据文本获取尺寸
    NSMutableAttributedString *attrStr = [[NSMutableAttributedString alloc] initWithString:self.badgeText ?: @"0"];
    
    NSRange allRange = [self.badgeText rangeOfString:self.badgeText];
    [attrStr addAttribute:NSFontAttributeName
                    value:self.badgeTextFont
                    range:allRange];
//    [attrStr addAttribute:NSForegroundColorAttributeName
//                    value:self.badgeTextColor
//                    range:allRange];
    NSStringDrawingOptions options =  NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading;
    
   return [attrStr boundingRectWithSize:CGSizeMake(MAXFLOAT, kBadgeHeight)
                                        options:options
                                        context:nil].size;
...
//                             alignment:NSTextAlignmentCenter];
            ...
            NSDictionary *dict = @{NSFontAttributeName :self.badgeTextFont,
                                   NSParagraphStyleAttributeName:paragraph,NSForegroundColorAttributeName:[UIColor whiteColor]
                                   };
           [self.badgeText drawInRect:textFrame withAttributes:dict];
