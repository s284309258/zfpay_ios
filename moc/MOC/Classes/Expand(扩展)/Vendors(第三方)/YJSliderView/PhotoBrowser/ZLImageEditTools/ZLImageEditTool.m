...
- (void)setEditImage:(UIImage *)editImage
{
    ...
    //如果强制剪裁，则不需要下方工具栏，由于filter不好限制其大小，故将其bottomView全部隐藏
    if(_configuration.mustClipImage && _editImage && _configuration.allowEditImage && _configuration.maxSelectCount == 1) {
        [self clipBtn_click];
        self.filterBtn.hidden = YES;
        self.rotateBtn.hidden = YES;
        self.clipBtn.hidden = YES;
        [self.rotateRatioBtn superview].hidden = YES;
    }
//    else {
//        self.filterBtn.hidden = NO;
//        self.rotateBtn.hidden = NO;
//        self.clipBtn.hidden = NO;
//        [self.rotateRatioBtn superview].hidden = NO;
//    }
}
