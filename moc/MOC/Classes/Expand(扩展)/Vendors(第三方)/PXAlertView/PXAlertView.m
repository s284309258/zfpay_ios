         ...
         completion:(PXAlertViewCompletionBlock)completion
{
    ...
    if (self) {
        ...
        CGRect rct  = (CGRect){AlertViewContentMargin, 0,
            self.messageScrollView.frame.size};
        rct.size.width -= AlertViewContentMargin;
        self.messageLabel.frame = rct;
        if (![messageLbl isKindOfClass:[MXTimeDownLabel class]]) {
            self.messageLabel.backgroundColor = [UIColor clearColor];
            self.messageLabel.textColor = [UIColor blackColor];
            self.messageLabel.textAlignment = NSTextAlignmentLeft;
            self.messageLabel.font = [UIFont font14];
         ...
         completion:(PXAlertViewCompletionBlock)completion
{
    ...
    if (self) {
        ...
        self.messageLabel = [[UILabel alloc] initWithFrame:(CGRect){AlertViewContentMargin, 0,
            self.messageScrollView.frame.size}];
...
- (void)keyboardWillShown:(NSNotification*)notification
{
    if(self.isVisible)
    {
        ...
        if ([[[UIDevice currentDevice] systemVersion] floatValue] < 8 && ([[UIApplication sharedApplication] statusBarOrientation] == UIInterfaceOrientationLandscapeLeft || [[UIApplication sharedApplication] statusBarOrientation] == UIInterfaceOrientationLandscapeRight)) {
...
- (void)dismiss:(id)sender animated:(BOOL)animated
{
    ...
    [UIView animateWithDuration:(animated ? 0.2 : 0) animations:^{
        ...
        if (self.completion) {
            ...
            if (sender) {
                if ([self.messageLabel isKindOfClass:[MXTimeDownLabel class]]) {
                    [((MXTimeDownLabel*)self.messageLabel) clear];
                }
...
+ (instancetype)showAlertWithTitle:(NSString *)title
                       ...
                       cancelTitle:(NSString *)cancelTitle
                        ...
                        completion:(PXAlertViewCompletionBlock)completion
{
    PXAlertView *alertView = [[self alloc] initWithTitle:title messageLbl:messageLbl cancelTitle:cancelTitle otherTitles:(otherTitle) ? @[ otherTitle ] : nil buttonsShouldStack:NO contentView:nil completion:completion];
    [alertView show];
    return alertView;
}
...
+ (instancetype)showAlertWithTitle:(NSString *)title
                       ...
                       cancelTitle:(NSString *)cancelTitle
                        ...
                        completion:(PXAlertViewCompletionBlock)completion
{
    MXTimeDownLabel *message = [[MXTimeDownLabel alloc] initWithFrame:CGRectZero];
    message.numberOfLines = 0;
    message.endTime = endTime;
    message.textColor = [UIColor redColor];
    message.text = @"剩余时间：0分0秒";
    
    PXAlertView *alertView = [[self alloc] initWithTitle:title messageLbl:message cancelTitle:cancelTitle otherTitles:(otherTitle) ? @[ otherTitle ] : nil buttonsShouldStack:NO contentView:nil completion:completion];
    [alertView show];

    @weakify(alertView)
    message.timeOver = ^(void) {
        @strongify(alertView)
        [alertView dismiss];
    };
...
- (NSInteger)addButtonWithTitle:(NSString *)title
{
    ...
    if (!self.cancelButton) {
        ...
        lastButton.titleLabel.font = [UIFont font17];
        ...
        self.verticalLine.frame = CGRectMake((AlertViewWidth-.5f)/2, self.buttonsY, AlertViewLineLayerWidth, AlertViewButtonHeight);
        ...
        self.cancelButton.titleLabel.font = [UIFont font17];
    }
