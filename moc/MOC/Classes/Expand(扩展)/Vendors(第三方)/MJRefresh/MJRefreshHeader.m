...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        // 多语言
        //[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(languageChangeNotification) name:kChangeLanguageNotification object:nil];
...
{
    MJRefreshHeaderStateIdleText = @"下拉可以刷新";
    MJRefreshHeaderStatePullingText = @"松开立即刷新";
    MJRefreshHeaderStateRefreshingText = @"正在刷新数据中...";
    [self setTitle:MJRefreshHeaderStateIdleText forState:MJRefreshHeaderStateIdle];
    [self setTitle:MJRefreshHeaderStatePullingText forState:MJRefreshHeaderStatePulling];
    [self setTitle:MJRefreshHeaderStateRefreshingText forState:MJRefreshHeaderStateRefreshing];
    [self setUpdatedTime:_updatedTime];
}
...
//            formatter.dateFormat =[MXLang(@"Public_Today", @"今天") stringByAppendingString:@" HH:mm"];
        ...
        if ([cmp1 day] == [cmp2 day]) { // 今天
            time=[@"今天" stringByAppendingString:time];
        }
        // 3.显示日期
