...
- (MJRefreshGifHeader *)addGifHeader
{
    ...
    NSMutableArray *gifImages = [NSMutableArray arrayWithCapacity:177];
    for (int imageIndex = 1; imageIndex < 178; imageIndex += 2) {
        [gifImages safeAddObj:[UIImage imageNamed:[NSString stringWithFormat:@"图层%@",@(imageIndex)]]];
    }
    
    [header setImages:gifImages forState:MJRefreshHeaderStateIdle];
    [header setImages:gifImages forState:MJRefreshHeaderStatePulling];
    [header setImages:gifImages forState:MJRefreshHeaderStateRefreshing];
