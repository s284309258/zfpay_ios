...
-(void)example01{
    ...
    @weakify(self)
    [self getAppImgList:@{@"img_type":@"04"} completion:^(id array, NSString *error) {
        @strongify(self)
        NSArray* tmp = array;
        if (tmp && tmp.count > 0) {
            AppImgModel* model = tmp[0];
            NSString* url = model.img_url;
            if (![url hasPrefix:@"http:"]) {
                url = [NSString stringWithFormat:@"%@/%@",[QNManager shared].qnHost,url];
            }
            [self showRunPhoto:url];
        }
    }];
...
-(void)showRunPhoto:(NSString* )url{
...
-(BOOL)xhLaunchAd:(XHLaunchAd *)launchAd clickAtOpenModel:(id)openModel clickPoint:(CGPoint)clickPoint{
    ...
    XHLWebViewController *VC = [[XHLWebViewController alloc] init];
