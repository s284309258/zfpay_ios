...
+ (void )groupIconWithImageArray:(NSArray *)URLArray bgColor:(UIColor *)bgColor image:(UIImageView* )image key:(NSString *)key{
    ...
    dispatch_group_notify(group, queue, ^{
        ...
        Block_Exec_Main_Async_Safe(^{
            UIImage* localImage = [UIImage groupIconWith:imageArray bgColor:[UIColor groupTableViewBackgroundColor]];
            image.image = localImage;
            NSString* imagePath = @"";
            if (!localImage) {
                imagePath = @"avatar_default";
            }else{
                //imagePath = [TalkManager saveChatImage:localImage];
            }
