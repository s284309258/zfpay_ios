...
@property(nonatomic,readwrite,strong)void (^oldIndex)(BankModel* index);
...
-(void)popOverSource:(NSArray*)dataArray index:(void (^)(BankModel* index))index{
...
-(void)inputChange:(NSNotification*)notification{
    if (notification.name == UITextFieldTextDidChangeNotification && notification.object == self) {
        ...
        [ProfitHelper searchLikeBank:@{@"key_word":self.text} completion:^(id array, NSString *error) {
            
            if (array ) {
                self.dataArray = array;
                NSPredicate *predicate = [NSPredicate predicateWithFormat:[NSString stringWithFormat:@"bank_name CONTAINS '%@'",[self replacingCharacter:self.text]]];
                 ...
                 */
//                array = [array sortedArrayUsingComparator:^NSComparisonResult(BankModel *obj1, BankModel *obj2) {
//                    NSComparisonResult result = NSOrderedSame;
//                    
//                    NSRange range1 = [obj1.bank_name rangeOfString:self.text];
//                    NSRange range2 = [obj2.bank_name rangeOfString:self.text];
...
-(void)changeInputSource:(NSArray*)changeArray{
    ...
    if (self.scrollView) {
        ...
        [self.superview.superview.superview.superview  addSubview:self.bgView];
//        [self.superview addSubview:self.bgView];
    ...
    if (CGRectIsEmpty(self.rectXY)) {
        UIWindow *window = [[UIApplication sharedApplication] keyWindow];
        UIView* tmp = self.superview.superview.superview.superview
        ;
        CGRect rect = [self.superview convertRect:self.frame toView:tmp];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([obj isKindOfClass:[BankModel class]]) {
        BankModel * model = obj;
        cell.textLabel.attributedText = [self attributedWithString:model.bank_name];
    }else{
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([onj isKindOfClass:[BankModel class]]) {
        self.text = [onj valueForKeyPath:@"bank_name"];
    }else{
...
-(NSMutableAttributedString*)attributedWithString:(NSString*)string {
    ...
    [attributed addFont:[UIFont font14] substring:string];
