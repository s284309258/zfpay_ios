...
- (UIImage*)grayScaleImageWithAlphaPng:(UIColor *)backgroundColor
{
    //先将带透明度的图片转换成指定的背景色
    UIImageView *tempImgView = [[UIImageView alloc] initWithImage:self];
    tempImgView.backgroundColor = backgroundColor;
    
    UIGraphicsBeginImageContextWithOptions(self.size, NO, [[UIScreen mainScreen] scale]);
    [tempImgView.layer renderInContext:UIGraphicsGetCurrentContext()];
    UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    
    CGRect imageRect = CGRectMake(0, 0, self.size.width, image.size.height);
