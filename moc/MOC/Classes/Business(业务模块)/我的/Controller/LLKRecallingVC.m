...
@interface LLKRecallingVC ()<UITableViewDelegate,UITableViewDataSource>
...
-(void)initData{
    [self getRecallTrafficCardList:@{@"status":@"00"} completion:^(id array, NSString *error) {
        if (array) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [view reload:isSelected?@"勾选":@"" title:[NSString stringWithFormat:@"SN码:%@",model.card_no] desc:model.cre_datetime];
...
-(RecallBottomView*)bottom{
    if (!_bottom) {
        ...
        _bottom.agreeBlock = ^(id data) {
            @strongify(self)
            [self dealRecallTrafficCard:@{@"status":@"09",@"ids_list":[self getPosStr:self.selectArray]} completion:^(BOOL success, NSString *error) {
                if (success) {
                    [self initData];
                }
            }];
        ...
        _bottom.disagreeBlock = ^(id data) {
            @strongify(self)
            [self dealRecallTrafficCard:@{@"status":@"08",@"ids_list":[self getPosStr:self.selectArray]} completion:^(BOOL success, NSString *error) {
                if (success) {
                    [self initData];
                }
            }];
