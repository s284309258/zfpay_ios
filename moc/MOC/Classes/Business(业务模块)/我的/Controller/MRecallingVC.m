...
-(void)initData{
    [self getRecallMposList:@{@"status":@"00"} completion:^(id array, NSString *error) {
        if (array) {
...
-(RecallBottomView*)bottom{
    if (!_bottom) {
        ...
        _bottom.agreeBlock = ^(id data) {
            @strongify(self)
            [self dealRecallMpos:@{@"status":@"09",@"ids_list":[self getPosStr:self.selectArray]} completion:^(BOOL success, NSString *error) {
                if (success) {
                    [self initData];
                }
            }];
        ...
        _bottom.disagreeBlock = ^(id data) {
            @strongify(self)
            [self dealRecallMpos:@{@"status":@"08",@"ids_list":[self getPosStr:self.selectArray]} completion:^(BOOL success, NSString *error) {
                if (success) {
                    [self initData];
                }
            }];
