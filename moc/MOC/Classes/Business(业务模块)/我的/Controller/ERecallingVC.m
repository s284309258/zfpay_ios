...
-(void)initData{
    [self getRecallTraditionalPosList:@{@"status":@"00",@"pos_type":@"epos"} completion:^(id array, NSString *error) {
        if (array) {
...
-(RecallBottomView*)bottom{
    if (!_bottom) {
        ...
        _bottom.agreeBlock = ^(id data) {
            @strongify(self)
            [self dealRecallTraditionalPos:@{@"status":@"09",@"ids_list":[self getPosStr:self.selectArray],@"pos_type":@"epos"} completion:^(BOOL success, NSString *error) {
                if (success) {
                    [self initData];
                }
            }];
        ...
        _bottom.disagreeBlock = ^(id data) {
            @strongify(self)
            [self dealRecallTraditionalPos:@{@"status":@"08",@"ids_list":[self getPosStr:self.selectArray],@"pos_type":@"epos"} completion:^(BOOL success, NSString *error) {
                if (success) {
                    [self initData];
                }
            }];
