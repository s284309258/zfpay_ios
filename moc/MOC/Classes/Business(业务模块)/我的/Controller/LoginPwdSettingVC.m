...
- (void)viewDidLoad {
    ...
    [self setNavBarTitle:Lang(@"修改登录密码")];
    ...
    self.data = @[
                  @{@"title":Lang(@"当前绑定手机号:"),@"placeholder":Lang(@"请输入手机号码")},
                      @{@"title":Lang(@"图形验证码"),@"placeholder":Lang(@"请输入验证码")},
                      @{@"title":Lang(@"短信验证码"),@"placeholder":Lang(@"请输入验证码")},
                      @{@"title":Lang(@"新登录密码"),@"placeholder":Lang(@"设置新登录密码")},
                      @{@"title":Lang(@"确认密码"),@"placeholder":Lang(@"再次确认密码")}
                      ];
...
-(UIView*)getViewFromIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 3) {
        ...
        [self.rePwdView isHiddenLine:YES];
...
- (void)sendSmsCode {
    ...
    [param setValue:@"FrontModifyLoginPass" forKey:@"bus_type"];
...
-(void)goTo:(id)sender{
    ...
    [PersonalCenterHelper modifyLoginPass:@{@"login_password":self.model.password,@"sms_code":self.model.captcha,@"token":AppUserModel.token} completion:^(BOOL success, NSString *error) {
