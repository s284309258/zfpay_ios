...
@interface UpdatePhoneVC ()<UITableViewDelegate, UITableViewDataSource>
...
- (void)viewDidLoad {
    ...
    [self setNavBarTitle:Lang(@"修改手机号码")];
    //self.edgesForExtendedLayout = UIRectEdgeAll;
    self.isShowBackButton = YES;
    
    self.model = [[LoginRegModel alloc]init];
    self.model.phoneNo = AppUserModel.user_tel;
    //    if ([AppUserModel.register_type integerValue] == 1) {
    //        self.model.type = ForgetPhoneType;
    //        self.model.phoneNo = AppUserModel.user_tel;
    self.data = @[
                  @{@"title":Lang(@"当前绑定手机号码:"),@"placeholder":Lang(@"请输入手机号码")},
                  @{@"title":Lang(@"图形验证"),@"placeholder":Lang(@"请输入验证码")},
                  @{@"title":Lang(@"短信验证号码"),@"placeholder":Lang(@"请输入验证码")}
                  ];
    //    }else{
    //         self.model.type = ForgetEmailType;
    //        self.model.email = AppUserModel.user_email;
    //        self.data = @[@[
    //                          @{@"title":Lang(@"邮箱号码"),@"placeholder":Lang(@"请输入邮箱号码")},
    //                          @{@"title":Lang(@"图形验证"),@"placeholder":Lang(@"请输入验证码")},
    //                          @{@"title":Lang(@"验证号码"),@"placeholder":Lang(@"请输入验证码")}
    //                          ],
    //                      @[
    //                          @{@"title":Lang(@"新交易密码"),@"placeholder":Lang(@"请输入交易密码")},
    //                          @{@"title":Lang(@"确认新密码"),@"placeholder":Lang(@"请再次输入交易密码")}
    //                          ]];
    //    }
...
- (void)sendSmsCode {
    ...
    [param setValue:@"FrontModifyTelFirst" forKey:@"bus_type"];
...
-(void)goTo:(id)sender{
    ...
    [PersonalCenterHelper modifyTelFirst:@{@"sms_code":self.model.captcha,@"token":AppUserModel.token} completion:^(id object, NSString *error) {
        if (object) {
            [MXRouter openURL:@"lcwl://BindPhonevC" parameters:@{@"valid_flag":object}];
        }
    }];
}
