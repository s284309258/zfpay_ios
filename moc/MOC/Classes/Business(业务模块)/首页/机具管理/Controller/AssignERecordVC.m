...
@interface AssignERecordVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation AssignERecordVC
...
-(void)initData{
    @weakify(self)
    [self selectPosBatchAllocate:@{@"last_id":self.last_id,@"key_word":self.keyword,@"pos_type":@"epos"} completion:^(id array, NSString *error) {
        @strongify(self)
        if (array) {
