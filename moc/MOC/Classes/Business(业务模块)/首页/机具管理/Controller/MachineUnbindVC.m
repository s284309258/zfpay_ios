...
@interface MachineUnbindVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation MachineUnbindVC
...
-(void)setupUI{
    ...
    self.typeArray = @[@"MPOS",@"传统POS",@"EPOS"];
...
-(UIButton*)bottomBtn{
    if (!_bottomBtn) {
        ...
        [_bottomBtn setTitle:@"提交申请" forState:UIControlStateNormal];
...
-(void)next:(id)sender{
    ...
    if (index == 0) {
        [self unbindMpos:@{@"sn":[self getPosStr:self.selectArray]} completion:^(BOOL success, NSString *error) {
            [NotifyHelper hideAllHUDsForView:self.view animated:YES];
            if (success) {
                [self requestUnbindList];
            }
        }];
    }else if(index == 1){
        [self unbindTraditionalPos:@{@"sn":[self getPosStr:self.selectArray]} completion:^(BOOL success, NSString *error) {
            [NotifyHelper hideAllHUDsForView:self.view animated:YES];
            if (success) {
                [self requestUnbindList];
            }
        ...
        [self unbindTraditionalPos:@{@"sn":[self getPosStr:self.selectArray],@"pos_type":@"epos"} completion:^(BOOL success, NSString *error) {
           [NotifyHelper hideAllHUDsForView:self.view animated:YES];
           if (success) {
               [self requestUnbindList];
           }
...
-(void)requestUnbindList{
    NSInteger index = [self.typeArray indexOfObject:self.typeValue];
    if (index == 0) {
        [self selectUnbindMpos:@{@"start_number":self.start_number,@"end_number":self.end_number} completion:^(id array, NSString *error) {
            if (array) {
        ...
        [self selectUnbindTraditionalPos:@{@"start_number":self.start_number,@"end_number":self.end_number} completion:^(id array, NSString *error) {
            if (array) {
                [self.selectArray removeAllObjects];
                self.dataSource = array;
                [self.tableView reloadData];
        ...
        [self selectUnbindTraditionalPos:@{@"start_number":self.start_number,@"end_number":self.end_number,@"pos_type":@"epos"} completion:^(id array, NSString *error) {
            if (array) {
                [self.selectArray removeAllObjects];
                self.dataSource = array;
                [self.tableView reloadData];
...
-(UIView *)tableHeaderView{
    if (!_tableHeaderView) {
        ...
        TradeHBItemView* typeView = [[TradeHBItemView alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH,40) column:3];
