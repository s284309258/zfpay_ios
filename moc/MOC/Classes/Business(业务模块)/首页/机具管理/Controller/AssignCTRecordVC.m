...
@interface AssignCTRecordVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation AssignCTRecordVC
...
-(void)initData{
    @weakify(self)
    [self selectPosBatchAllocate:@{@"last_id":self.last_id,@"key_word":self.keyword,@"pos_type":@"TraditionalPOS"} completion:^(id array, NSString *error) {
        @strongify(self)
        if (array) {
