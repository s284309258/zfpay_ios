...
@interface CTUnbindApplyRecordVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation CTUnbindApplyRecordVC
...
-(void)initData{
    @weakify(self)
    NSMutableDictionary* param = [NSMutableDictionary dictionaryWithDictionary:@{@"last_id":self.last_id,@"key_word":self.keyword}];
    if ([self.type isEqualToString:@"EPOS"]) {
        [param setObject:@"epos" forKey:@"pos_type"];
    }
    [self getTraditionalPosUnbindRecordList:param completion:^(id array, NSString *error) {
        @strongify(self)
        if (array) {
