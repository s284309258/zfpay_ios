...
@interface ApplyRateMRecordVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ApplyRateMRecordVC
...
-(void)initData{
    ...
    [self.tableView addLegendHeaderWithRefreshingBlock:^{
        @strongify(self)
        self.last_id = @"";
        [self getApplyRateMposRecordList:@{@"last_id":self.last_id,@"key_word":self.key_word} completion:^(id array, NSString *error) {
    ...
    [self getApplyRateMposRecordList:@{} completion:^(id array, NSString *error) {
        if (array) {
            self.dataSource = array;
            [self.tableView reloadData];
        }
