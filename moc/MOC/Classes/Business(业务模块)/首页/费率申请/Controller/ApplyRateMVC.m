...
@interface ApplyRateMVC ()<UITableViewDelegate,UITableViewDataSource>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    switch (indexPath.section) {
        ...
        case 1:{
            ...
            [cell reload:model select:isSelected type:@"MPOS"];
            return cell;
        }
        default:
            break;
...
-(void)requestAllocationList{
    [self getApplyRateMposList:@{@"start_number":self.start_number,@"end_number":self.end_number} completion:^(id array, NSString *error) {
        if (array) {
            [self.selectArray removeAllObjects];
