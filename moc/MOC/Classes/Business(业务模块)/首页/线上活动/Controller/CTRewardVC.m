...
@interface CTRewardVC ()<UITableViewDelegate,UITableViewDataSource>
...
-(void)initData{
    ...
    [self.tableView addLegendHeaderWithRefreshingBlock:^{
        @strongify(self)
        self.last_id = @"";
        [self getTraditionalPosActivityApplyList:@{@"last_id":self.last_id} completion:^(id array, NSString *error) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    TraditionalPosActivityApplyModel* model = self.dataSource[indexPath.section];
    [MXRouter openURL:@"lcwl://ActivitiesOrderDetailVC" parameters:@{@"apply_id":model.apply_id,@"type":@"0"}];
}
