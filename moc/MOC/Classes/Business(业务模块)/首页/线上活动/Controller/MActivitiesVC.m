...
@interface MActivitiesVC ()<UITableViewDelegate,UITableViewDataSource>
...
-(void)initData{
    ...
    [self.tableView addLegendHeaderWithRefreshingBlock:^{
        [self getMposOnlineActivityList:@{} completion:^(id array, NSString *error) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    TraditionalPosActivityModel* model = self.dataSource[indexPath.row];
    [MXRouter openURL:@"lcwl://ActivityVC" parameters:@{@"activity_id":model.activity_id,@"type":@"1"}];
}
