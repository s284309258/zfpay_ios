...
@interface CTAgentAddressBookDetailVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation CTAgentAddressBookDetailVC
...
-(void)initUI{
    ...
    [self.tableView addLegendHeaderWithRefreshingBlock:^{
        @strongify(self)
        self.last_id = @"";
        [self getReferAgencyTraditionalPosList:@{@"user_id":self.model.user_id,@"last_id":self.last_id,@"trade_status":self.trade_status} completion:^(id array, NSString *error) {
...
-(void)requestMonthData{
    ...
    [self getReferAgencyTraditionalPosTradeAmountAvg:@{@"cre_month":month_param,@"user_id":self.model.user_id} completion:^(id object, NSString *error) {
        if (object) {
...
-(void)initData{
    self.dataSource = @[];
    @weakify(self)
    [self getReferAgencyHeadTraditionalPosInfo:@{@"user_id":self.model.user_id} completion:^(id object, NSString *error) {
        @strongify(self)
        if (object) {
...
//            NSString* name = [NSString stringWithFormat:@"%@(%@)\n机具数量:%@ 已激活:%@ 未激活:%@",self.model.user_tel,self.model.real_name,self.headerDict.tra_pos_num,self.headerDict.tra_act_num,self.headerDict.tra_inact_num];
            ...
            {
                NSString* str1 = [NSString stringWithFormat:@"传统POS共计%@台",self.headerDict.tra_pos_num];
            ...
            {
                NSString* str1 = [NSString stringWithFormat:@" 已激活%@",self.headerDict.tra_act_num];
                 NSMutableAttributedString* attr1 = [[NSMutableAttributedString alloc]initWithString:str1];
                 [attr1 addColor:[UIColor lightGrayColor] substring:@"已激活"];
                 [attr1 addFont:[UIFont font11] substring:@"已激活"];
                 [attr1 addColor:[UIColor moGreen] substring:self.headerDict.tra_act_num];
                 [attr1 addFont:[UIFont font11] substring:self.headerDict.tra_act_num];
                 [attr appendAttributedString:attr1];
           }
            {
                 NSString* str1 = [NSString stringWithFormat:@" 未激活%@",self.headerDict.tra_inact_num];
                  NSMutableAttributedString* attr1 = [[NSMutableAttributedString alloc]initWithString:str1];
                  [attr1 addColor:[UIColor lightGrayColor] substring:@"未激活"];
                  [attr1 addFont:[UIFont font11] substring:@"未激活"];
                  [attr1 addColor:[UIColor moRed] substring:self.headerDict.tra_inact_num];
                  [attr1 addFont:[UIFont font11] substring:self.headerDict.tra_inact_num];
                  [attr appendAttributedString:attr1];
            }
