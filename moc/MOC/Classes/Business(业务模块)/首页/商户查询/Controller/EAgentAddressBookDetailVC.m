...
@interface EAgentAddressBookDetailVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation EAgentAddressBookDetailVC
...
-(void)initUI{
    ...
    [self.tableView addLegendHeaderWithRefreshingBlock:^{
        @strongify(self)
        self.last_id = @"";
        [self getReferAgencyTraditionalPosList:@{@"user_id":self.model.user_id,@"last_id":self.last_id,@"trade_status":self.trade_status,@"pos_type":@"epos"} completion:^(id array, NSString *error) {
            [self.tableView.header endRefreshing];
            if (array) {
                self.dataSource = array;
                [self.tableView reloadData];
                ReferAgencyPosModel* model = self.dataSource.lastObject;
                self.last_id = model.trapos_id;
            }
...
-(void)requestMonthData{
    ...
    [self getReferAgencyTraditionalPosTradeAmountAvg:@{@"cre_month":month_param,@"user_id":self.model.user_id,@"pos_type":@"epos"} completion:^(id object, NSString *error) {
        if (object) {
...
-(void)initData{
    self.dataSource = @[];
    @weakify(self)
    [self getReferAgencyHeadTraditionalPosInfo:@{@"user_id":self.model.user_id,@"pos_type":@"epos"} completion:^(id object, NSString *error) {
        @strongify(self)
        if (object) {
            self.headerDict = object;
//            NSString* name = [NSString stringWithFormat:@"%@(%@)\n机具数量:%@ 已激活:%@ 未激活:%@",self.model.user_tel,self.model.real_name,self.headerDict.tra_pos_num,self.headerDict.tra_act_num,self.headerDict.tra_inact_num];
            NSString* name = [NSString stringWithFormat:@"%@\n%@",self.model.real_name,self.model.user_tel];
            NSMutableAttributedString* attr = [[NSMutableAttributedString alloc]initWithString:@""];
            {
                
                NSString* e_pos_num = [StringUtil isEmpty:self.headerDict.e_pos_num]?@"0":self.headerDict.e_pos_num;
                NSString* str1 = [NSString stringWithFormat:@"EPOS共计%@台",e_pos_num];
            ...
            {
                NSString* e_act_num = [StringUtil isEmpty:self.headerDict.e_act_num]?@"0":self.headerDict.e_act_num;
                NSString* str1 = [NSString stringWithFormat:@" 已激活%@",e_act_num];
                 NSMutableAttributedString* attr1 = [[NSMutableAttributedString alloc]initWithString:str1];
                 [attr1 addColor:[UIColor lightGrayColor] substring:@"已激活"];
                 [attr1 addFont:[UIFont font11] substring:@"已激活"];
                 [attr1 addColor:[UIColor moGreen] substring:e_act_num];
                 [attr1 addFont:[UIFont font11] substring:e_act_num];
                 [attr appendAttributedString:attr1];
           }
            {
                NSString* e_inact_num = [StringUtil isEmpty:self.headerDict.e_inact_num]?@"0":self.headerDict.e_inact_num;
                 NSString* str1 = [NSString stringWithFormat:@" 未激活%@",e_inact_num];
                  NSMutableAttributedString* attr1 = [[NSMutableAttributedString alloc]initWithString:str1];
                  [attr1 addColor:[UIColor lightGrayColor] substring:@"未激活"];
                  [attr1 addFont:[UIFont font11] substring:@"未激活"];
                  [attr1 addColor:[UIColor moRed] substring:e_inact_num];
                  [attr1 addFont:[UIFont font11] substring:e_inact_num];
                  [attr appendAttributedString:attr1];
            }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
     ReferAgencyPosModel* model = self.dataSource[indexPath.row];
    [MXRouter openURL:@"lcwl://MerchantDetailVC" parameters:@{@"type":@"CTPOS",@"sn":model.sn}];
}
@end
