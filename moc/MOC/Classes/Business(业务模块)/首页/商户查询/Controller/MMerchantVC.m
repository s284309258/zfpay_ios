...
#import "MMerchantVC.h"
#import "MerchantCell.h"
#import "NSObject+Home.h"
static NSString* reuseIdentifierBar = @"reuseIdentifierBar";
@interface MMerchantVC ()<UITableViewDelegate,UITableViewDataSource>
...
-(void)initData{
    [self getSummaryMposList:@{} completion:^(id dict, NSString *error) {
        if (dict) {
            self.summaryDict = dict;
            self.dataSource = [[NSMutableArray alloc]initWithArray:@[
            @{@"image":@"全部商户",@"title":Lang(@"全部商户"),@"desc":Lang(@"发展的所有商户"),@"right":self.summaryDict[@"all_merchant"],@"vc":@"lcwl://MMerchantListVC"},
            @{@"image":@"优质商户",@"title":Lang(@"优质商户"),@"desc":Lang(@"本月交易金额>=50000元"),@"right":self.summaryDict[@"excellent_merchant"],@"vc":@"lcwl://MMerchantListVC"},
            @{@"image":@"活跃商户",@"title":Lang(@"活跃商户"),@"desc":Lang(@"近30天交易笔数≥2笔 并 交易金额≥10元"),@"right":self.summaryDict[@"active_merchant"],@"vc":@"lcwl://MMerchantListVC"},
            @{@"image":@"休眠商户",@"title":Lang(@"休眠商户"),@"desc":Lang(@"入网>=60天,连续无交易>60天"),@"right":self.summaryDict[@"dormant_merchant"],@"vc":@"lcwl://MMerchantListVC"},
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (![StringUtil isEmpty:vcString]) {
        if ([vcString isEqualToString:@"lcwl://CTStandardMerchantVC"]) {
            [MXRouter openURL:vcString parameters:@{@"pos_type":@"02"}];
            return;
        }
        [MXRouter openURL:vcString parameters:@{@"type":@(indexPath.row),@"naviTitle":dict[@"title"]}];
    }
}
