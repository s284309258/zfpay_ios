...
#import "PosInputInquiryunCheckVC.h"
...
@interface PosInputInquiryunCheckVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation PosInputInquiryunCheckVC
...
-(void)initData{
    ...
    [self.tableView addLegendHeaderWithRefreshingBlock:^{
        @strongify(self)
        self.last_id = @"";
        NSMutableDictionary* param = [NSMutableDictionary dictionaryWithDictionary:@{@"last_id":self.last_id,@"key_word":self.key_word,@"biz_code":@"01"}];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell reload:tmp type:@"uncheck"];
    return cell;
}

- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
     PosInputInquiryModel* tmp = self.dataSource[indexPath.row];
    [MXRouter openURL:@"lcwl://PosInputInquiryunCheckDetailVC" parameters:@{@"model":tmp}];
}
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText{
    ...
    [self getTraditionalPosInstallList:@{@"last_id":self.last_id,@"key_word":self.key_word,@"biz_code":@"01"} completion:^(id array, NSString *error) {
        if (array) {
            self.dataSource = array;
            [self.tableView reloadData];
            PosInputInquiryModel* lastObject = self.dataSource.lastObject;
