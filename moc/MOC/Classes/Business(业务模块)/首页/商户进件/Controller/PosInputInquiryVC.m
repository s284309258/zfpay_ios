...
@implementation PosInputInquiryVC
...
-(YNPageScrollMenuView*)menu{
    if(!_menu){
        ...
        _menu = [YNPageScrollMenuView pagescrollMenuViewWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 38) titles: @[@"审核通过",@"审核不通过"]  configration:configration delegate:self currentIndex:0];
    }
    return _menu;
}
...
-(void)initUI{
    ...
    [self setNavBarTitle:@"商户进件查询"];
    PosInputInquiryCheckVC* vc1 = [[PosInputInquiryCheckVC alloc]init];
    vc1.type = self.type;
    PosInputInquiryunCheckVC* vc2 = [[PosInputInquiryunCheckVC alloc]init];
    vc2.type = self.type;
