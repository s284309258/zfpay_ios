...
#define IconBackTime 0.25    //icon刷新完返回最顶端的时间
...
- (void)addObserverForView:(UIView *)view {
    [view addObserver:[self nextResponder] forKeyPath:@"contentOffset" options:NSKeyValueObservingOptionOld|NSKeyValueObservingOptionNew context:nil];
    self.isObserver = YES;
}

- (void)XD_observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    ...
    //NSLog(@"### XD_observeValueForKeyPath change before async: %@",change);
    /**异步调用主线程**/
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            __strong typeof(weakSelf) strongSelf = weakSelf;
            //NSLog(@"### XD_observeValueForKeyPath change after async: %@",change);
            /**非刷新状态**/
...
- (void)defaultHandleWithOffSet:(CGFloat)offsetY change:(NSDictionary<NSKeyValueChangeKey,id> *)change {
    ...
    if (defaultoffsetY < self.threshold && defaultoffsetY < 0) {
        [self.refreshView setContentOffset:CGPointMake(0, defaultoffsetY)];
                         ...
                         */
        //NSLog(@"### defaultHandleWithOffSet change: %@",change);
...
- (void)anmiationHandelwithChange:(NSDictionary<NSKeyValueChangeKey,id> *)change andStatus:(StatusOfRefresh)status needAnimation:(BOOL)need {
    ...
    /**
     非刷新状态下的动作处理
     */
    //NSLog(@"### anmiationHandelwithChange change: %@",change);
...
- (void)XD_freeReFresh {
    if (self.isObserver) {
        self.isObserver = NO;
        [self.extenScrollView removeObserver:[self nextResponder] forKeyPath:@"contentOffset"];
    }
}
