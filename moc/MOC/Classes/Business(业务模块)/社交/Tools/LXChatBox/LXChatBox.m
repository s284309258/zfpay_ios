...
- (void)setIsChatBar:(BOOL)isChatBar {
    _isChatBar = isChatBar;
    self.textView.x = 2 * BOXBTNSPACE;
    self.textView.width = KScreenW - CHATBOX_BUTTON_WIDTH - 4 *BOXBTNSPACE;
    
    self.faceButton.x = self.textView.maxX + BOXBTNSPACE;
}
...
-(void)setStatus:(LXChatBoxStatus)status{
    ...
    switch (_status) {
        case LXChatBoxStatusNothing:
        {
            ...
            break;
        case LXChatBoxStatusChatNothing:
        {
            self.voiceButton.selected = YES;
            self.faceView.hidden = self.moreView.hidden = YES;
            [self.textView resignFirstResponder];
