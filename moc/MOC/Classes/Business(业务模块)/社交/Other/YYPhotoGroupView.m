...
@property (nonatomic, strong) UIButton *saveBnt;
...
//    _saveBnt = [[UIButton alloc] init];
//    _saveBnt.frame = CGRectMake(SCREEN_WIDTH-65, SCREEN_HEIGHT-40, 50, 30);
//    [_saveBnt setTitle:@"保存" forState:UIControlStateNormal];
//    _saveBnt.titleLabel.font = [UIFont systemFontOfSize:14];
//    [_saveBnt setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
//    _saveBnt.backgroundColor = [UIColor colorWithRed:0.1f green:0.1f blue:0.1f alpha:0.90f];
//    _saveBnt.layer.cornerRadius = 2;
//    _saveBnt.clipsToBounds = YES;
//    [_saveBnt addTarget:self action:@selector(saveImage) forControlEvents:UIControlEventTouchUpInside];
//    [self addSubview:_saveBnt];
...
{
    if(error != NULL){
        [NotifyHelper showMessageWithMakeText:@"保存图片失败"];
    }else{
        [NotifyHelper showMessageWithMakeText:@"保存图片成功"];
    }
}
...
- (void)dismiss {
    [self dismissAnimated:YES completion:^{
        Block_Exec(self.dismissBlock,nil);
    }];
}
...
- (void)longPress:(UILongPressGestureRecognizer*)sender {
    ...
    if(sender.state == UIGestureRecognizerStateBegan) {
        ...
        UIAlertAction *action3 = [UIAlertAction actionWithTitle:@"识别二维码" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            NSString *result = [tile.imageView.image scanQRCode];
            if (![StringUtil isEmpty:result]) {
                Block_Exec(self.dismissBlock,result);
            } else {
                [NotifyHelper showMessageWithMakeText:@"未识别到二维码"];
            }
        }];
        ...
        [MoApp.window.rootViewController presentViewController:actionSheet animated:YES completion:nil];
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if (fabs(v.y) > 1000 || fabs(deltaY) > 120) {
                ...
                [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionCurveLinear | UIViewAnimationOptionBeginFromCurrentState animations:^{
                    ...
                    [self removeFromSuperview];
                    if (self.dismissBlock) self.dismissBlock(nil);
                }];
