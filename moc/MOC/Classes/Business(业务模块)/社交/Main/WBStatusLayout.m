...
//过滤不需要的数据
- (void)filterUselessData:(WBStatus *)status {
    status.title = nil;
    status.picBg = nil;
}
...
- (void)_layout {
    ...
    _picHeight = 0;
    _cardHeight = 0;
    _commentTextHeight = 0;
    _toolbarHeight = kWBCellToolbarHeight;
    _marginBottom = kWBCellMargin;
...
//    _height += _textHeight;
    _height += (_textHeight > 0 ? _textHeight - 28 : _textHeight); //正文内容向上偏移28个像素
    ...
    _height += _commentTextHeight;
    _height += kWBCellToolbarBottomMargin;
...
#endif
    nameText.font = [UIFont boldSystemFontOfSize:kWBCellNameFontSize];
    nameText.color = kWBCellNameNormalColor;//user.mbrank > 0 ? kWBCellNameOrangeColor : kWBCellNameNormalColor;
...
- (void)_layoutAddress {
    if(_status.circle_user_addr && _status.circle_user_addr.length > 0) {
        NSMutableAttributedString *addressText = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@" %@ ",_status.circle_user_addr]];
        addressText.font = [UIFont systemFontOfSize:13];
        addressText.color = [UIColor grayColor];//user.mbrank > 0 ? kWBCellNameOrangeColor : kWBCellNameNormalColor;
        //addressText.lineBreakMode = NSLineBreakByCharWrapping;
        //addressText.alignment = NSTextAlignmentCenter;
        ...
        [addressText setParagraphStyle:paragraphStyle];
        
        NSStringDrawingOptions options = NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading;
        CGRect rect = [addressText boundingRectWithSize:CGSizeMake(kWBCellNameWidth, CGFLOAT_MAX) options:options context:nil];
        YYTextContainer *container = [YYTextContainer containerWithSize:CGSizeMake(rect.size.width, 9999)];
        container.maximumNumberOfRows = 1;
        _addressTextLayout = [YYTextLayout layoutWithContainer:container text:addressText];
    } else {
        _addressTextLayout = nil;
    }
...
//    }
    ...
    if (sourceText.length == 0) {
        NSString *createTime = [WBStatusHelper stringWithTimelineDate:_status.createdAt];
        NSMutableAttributedString *createTimeAtt = [[NSMutableAttributedString alloc] initWithString:createTime];
        createTimeAtt.font = [UIFont systemFontOfSize:12];
        createTimeAtt.color = [UIColor colorWithHexString:@"#A0A0A0"];
        YYTextContainer *container = [YYTextContainer containerWithSize:CGSizeMake(kScreenWidth - 100, kWBCellTitleHeight)];
        _timeTextLayout = [YYTextLayout layoutWithContainer:container text:createTimeAtt];
    }
}
...
- (void)_layoutRetweet {
    ...
    if (_retweetPicHeight > 0) {
        ...
        _retweetHeight += kWBCellTopPadding;//kWBCellPadding;
...
/// 文本
- (void)_layoutText {
    _textHeight = 10; //如果没有正文的话，给一个默认值10
    _textLayout = nil;
    ...
    [text setTextHighlightRange:[text rangeOfAll] color:nil backgroundColor:[UIColor whiteColor] userInfo:@{@"content": text.string ?: @""} tapAction:nil longPressAction:nil];
...
- (void)_layoutPicsWithStatus:(WBStatus *)status isRetweet:(BOOL)isRetweet {
    ...
    CGFloat valideWidth1_2 = kWBCellContentWidth*0.5; //一张
    CGFloat valideWidth3 = kWBCellContentWidth-kWBCellPadding+kWBCellRightPadding; //两张照片以上
    
    CGFloat len1_3 = (valideWidth3 - kWBCellPaddingPic*3) / 3;
    ...
    switch (status.pics.count) {
        case 1: {
            ...
            CGFloat maxLen = valideWidth1_2;// / 2.0;
            if (pic.keepSize || bmiddle.width < 1 || bmiddle.height < 1) {
                ...
                if (bmiddle.width < bmiddle.height) {
                    picSize.width = maxLen;//(float)bmiddle.width / (float)bmiddle.height * maxLen;
        ...
        case 2: {
            picSize.width = len1_3;//(valideWidth3 - kWBCellPaddingPic)/2;
        ...
        case 4: {
            picSize.width = len1_3;//(valideWidth3 - kWBCellPaddingPic)/2;
            picSize.height = picSize.width;
            picHeight = picSize.height * 2 + kWBCellPaddingPic;
        } break;
        case 5: case 6: {
            picSize = CGSizeMake(len1_3, len1_3);
...
- (void)_layoutCardWithStatus:(WBStatus *)status isRetweet:(BOOL)isRetweet {
    ...
    if ((pageInfo.type == 2) && [pageInfo.objectType isEqualToString:@"video"]) {
        // 视频，一个大图片，上面播放按钮
        if (pageInfo.pagePic) {
            cardType = WBStatusCardTypeVideo;
            cardHeight = (kWBCellContentWidth - kWBCellPaddingPic)/2;//(2 * kWBCellContentWidth - kWBCellPaddingPic) / 3.0;
        }
    } else {
...
///点赞评论
- (void)_layoutComment {
    NSMutableAttributedString *comment = [[NSMutableAttributedString alloc] init];
    
    //转发次数
    if(self.status.repostsCount > 0) {
        NSMutableAttributedString *repostsTotal = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%d次转发\n",self.status.repostsCount]];
        repostsTotal.color = [UIColor blackColor];
        repostsTotal.font = [UIFont boldSystemFontOfSize:kWBCellTitlebarFontSize];
        [comment appendAttributedString:repostsTotal];
        
        YYTextHighlight *highlight = [YYTextHighlight new];
        highlight.userInfo = @{@"type":@"forward",@"circle_id":self.status.circle_id ?: @""};
        [repostsTotal setTextHighlight:highlight range:NSMakeRange(0, repostsTotal.string.length)];
    }
    
    
    NSMutableAttributedString *likePersons = [[NSMutableAttributedString alloc] initWithString:self.status.praise_info_list.count > 0 ? @"     " : @""];
    NSMutableAttributedString *comma = [[NSMutableAttributedString alloc] initWithString:@"、"];
    comma.color = [UIColor blackColor];
    
    if(self.status.attitudesCount > 0) {
//        NSMutableAttributedString *praiseTotal = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@" 等%d个赞",self.status.attitudesCount]];
//        praiseTotal.color = [UIColor blackColor];
//        praiseTotal.font = [UIFont boldSystemFontOfSize:kWBCellTitlebarFontSize];
        
        //点赞人员
        [self.status.praise_info_list enumerateObjectsUsingBlock:^(PraiseItem * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            if(obj.user_name == nil || obj.user_name.length == 0) {
                return;
            }
            //NSInteger curLength = praiseTotal.string.length;
            NSMutableAttributedString *personal = [[NSMutableAttributedString alloc] initWithString:obj.user_name];
            personal.color = [UIColor colorWithHexString:@"#85A6DD"];
            
            YYTextHighlight *highlight = [YYTextHighlight new];
            highlight.userInfo = @{@"type":@"personal",@"user_id":obj.user_id ?: @""};
            [personal setTextHighlight:highlight range:NSMakeRange(0, personal.length)];
            [likePersons appendAttributedString:personal];
            
            if(idx < self.status.praise_info_list.count-1) {
                [likePersons appendAttributedString:comma];
            }
        }];
        likePersons.font = [UIFont systemFontOfSize:kWBCellTitlebarFontSize];
        //[likePersons appendAttributedString:praiseTotal];
        [likePersons appendString:@"\n"];
    }
    
    //评论内容
    NSMutableAttributedString *commentPart = [[NSMutableAttributedString alloc] init];
    for(WBCommentItem *item in self.status.comment_info_list) {
        NSString *name = item.user_name ?: @"";
        NSString *content = item.content ?: @"";
        NSString *commentId = item.comment_id ?: @"";
        NSString *commentUserName = item.comment_user_name ?: @"";
        
        NSMutableAttributedString *commentStr = nil;
        if(commentId.length > 0 && commentUserName.length > 0) {
            commentStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@ 回复 %@ ：%@",name,commentUserName,content]];
            [commentStr setColor:[UIColor moBlue] range:[commentStr.string rangeOfString:commentUserName]];
            
            YYTextHighlight *highlightName = [YYTextHighlight new];
            highlightName.userInfo = @{@"type":@"personal",@"user_id":item.comment_user_id ?: @""};
            [commentStr setTextHighlight:highlightName range:[commentStr.string rangeOfString:commentUserName]];
        } else {
            commentStr = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@ ：%@",name,content]];
        }
        //container.linePositionModifier = modifier;
        [commentStr setColor:[UIColor moBlue] range:[commentStr.string rangeOfString:name]];
        [commentStr setColor:[UIColor blackColor] range:[commentStr.string rangeOfString:content]];
        
        YYTextHighlight *highlightName = [YYTextHighlight new];
        highlightName.userInfo = @{@"type":@"personal",@"user_id":item.user_id ?: @""};
        [commentStr setTextHighlight:highlightName range:[commentStr.string rangeOfString:name]];
        
        YYTextHighlight *highlightCotent = [YYTextHighlight new];
        highlightCotent.userInfo = @{@"type":@"commentContent",@"commentItem":[item modelToJSONObject] ?: @""};
        [commentStr setTextHighlight:highlightCotent range:[commentStr.string rangeOfString:content]];
        
        [commentPart appendAttributedString:commentStr];
        [commentPart appendString:@"\n"];
    }
    
    if(commentPart.string.length > 0) {
        commentPart.font = [UIFont systemFontOfSize:kWBCellTitlebarFontSize];
        
        if(self.status.commentsCount > 4) { //最多显示4条，超过4条才显示查看全部
            //查看全部评论
            NSMutableAttributedString *commentTotal = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"查看全部%d条评论",self.status.commentsCount]];
            commentTotal.font = [UIFont boldSystemFontOfSize:kWBCellTitlebarFontSize];
            [commentPart appendAttributedString:commentTotal];
        }
    }
    //[likePersons setLineSpacing:20 range:NSMakeRange(0, likePersons.string.length)];
    
    if(likePersons.string.length > 0) {
        ///在点赞信息前面添加点赞图标
        
        YYTextAttachment *attachment = [YYTextAttachment new];
        attachment.contentMode = UIViewContentModeScaleAspectFit;
        attachment.contentInsets = UIEdgeInsetsMake(2, 0, 0, 0);
        attachment.content = [UIImage imageNamed:@"点赞深蓝"];
        [likePersons setTextAttachment:attachment range:NSMakeRange(0, 5)];
        
        [comment appendAttributedString:likePersons];
    }
    
    if(commentPart.string.length > 0) {
        [comment appendAttributedString:commentPart];
    }
    
    //[comment appendAttributedString:[[NSMutableAttributedString alloc] initWithString:@"\n"]];

//    [comment appendAttributedString:comment2];
    //comment.font = [UIFont systemFontOfSize:kWBCellTitlebarFontSize];
    ...
    _commentTextLayout = [YYTextLayout layoutWithContainer:container text:comment];
    if (!_commentTextLayout) return;
    
    _commentTextHeight = [modifier heightForLineCount:_commentTextLayout.rowCount];
}
...
- (void)_layoutToolbar {
    ...
    NSString *count = [@(_status.awardCount) description];
    NSString *awardStr = _status.awardCount <= 0 ? @"" : (_status.user.userID == [AppUserModel.user_id intValue] ? [NSString stringWithFormat:@"%@人已打赏",count] : count);
    
    NSMutableAttributedString *awardText = [[NSMutableAttributedString alloc] initWithString:awardStr];
    awardText.font = font;
    _toolbarAwardTextLayout = [YYTextLayout layoutWithContainer:container text:awardText];
    _toolbarAwardTextWidth = CGFloatPixelRound(_toolbarAwardTextLayout.textBoundingRect.size.width);
}
...
- (NSMutableAttributedString *)_textWithStatus:(WBStatus *)status
    ...
    NSMutableString *string = isRetweet ? status.trans_content.mutableCopy : status.text.mutableCopy;
    if (string.length == 0) return nil;
    NSString *insertName = nil;
    if (isRetweet) {
        NSString *name = status.trans_user_name;
        if (name.length == 0) {
            name = status.user.screenName;
        }
        if (name) {
            insertName = [NSString stringWithFormat:@"%@：",name];
            [string insertString:insertName atIndex:0];
        }
    }
    // 字体
    ...
    if(isRetweet && status.trans_user_name) {
        //添加转发name的YYTextHighlight
        // 高亮状态
        YYTextHighlight *nameHighlight = [YYTextHighlight new];
        [nameHighlight setBackgroundBorder:highlightBorder];
        // 数据信息，用于稍后用户点击
        nameHighlight.userInfo = @{@"type":@"personal",@"user_id": [NSString stringWithFormat:@"%@",status.trans_user_id]};
        [text setTextHighlight:nameHighlight range:NSMakeRange(0, string.length)];
        NSRange nameRange = [text.string rangeOfString:insertName];
        [text setColor:[UIColor moBlue] range:nameRange];
    }
