...
- (BOOL)isTransfer {
    return [self.circle_send_type isEqualToString:@"01"];
}

- (NSInteger)circleFileType {
    NSString *imageType = self.isTransfer ? self.trans_image_type : self.circle_image_type;
    if(imageType == nil) {
        return 0;
    }
    NSString *type = [[imageType componentsSeparatedByString:@"|"] firstObject];
    return [type integerValue];
}

- (CGSize)circleFileSize {
    if(self.circle_image_type == nil) {
        return CGSizeZero;
    }
    
    NSString *sizeStr = [[self.circle_image_type componentsSeparatedByString:@"|"] safeObjectAtIndex:1];
    NSInteger width = [[[sizeStr componentsSeparatedByString:@"x"] firstObject] integerValue];
    NSInteger height = [[[sizeStr componentsSeparatedByString:@"x"] lastObject] integerValue];
    return CGSizeMake(width, height);
}

- (NSString *)circleFileSufix {
    if(self.circle_image_type == nil) {
        return @"";
    }
    
    NSString *sufixStr = [[self.circle_image_type componentsSeparatedByString:@"|"] lastObject];
    return [sufixStr pathExtension];
}
@end
