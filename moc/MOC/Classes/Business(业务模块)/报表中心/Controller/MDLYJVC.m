...
@interface MDLYJVC ()<UITableViewDelegate,UITableViewDataSource,PGDatePickerDelegate>
...
-(void)initData{
    ...
    [self getDayAgencyMposDetail];
    [self getMonthAgencyMposDetail];
    
    self.headerData = @[@{@"color":@"#01C088",@"text":@"月业绩",@"desc":@"2019-08"},
                        @{@"color":@"#01C088",@"text":@"日业绩",@"desc":@"2019-08-06"}];
}

-(void)getDayAgencyMposDetail{
    [self getDayAgencyMposDetail:@{@"date":[self.dayStr stringByReplacingOccurrencesOfString:@"-" withString:@""]} completion:^(id object, NSString *error) {
        if (object) {
            self.dayData = object;
...
-(void)getMonthAgencyMposDetail{
    [self getMonthAgencyMposDetail:@{@"date":[self.monthStr stringByReplacingOccurrencesOfString:@"-" withString:@""]} completion:^(id object, NSString *error) {
        if (object) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    view.chartBlock  = ^(id data) {
        ...
        if (indexPath.section == 0) {
            [MXRouter openURL:@"lcwl://PerformanceChartVC" parameters:@{@"type":@"MDL",@"subType":@"month",@"dateStr":[self.monthStr stringByReplacingOccurrencesOfString:@"-" withString:@""]}];
        }else{
            [MXRouter openURL:@"lcwl://PerformanceChartVC" parameters:@{@"type":@"MDL",@"subType":@"day",@"dateStr":[self.dayStr stringByReplacingOccurrencesOfString:@"-" withString:@""]}];
        }
    };
...
//{
//    if (datePicker.datePickerMode == PGDatePickerModeDate) {
//        self.dayStr = [NSString stringWithFormat:@"%ld-%ld-%ld",dateComponents.year,dateComponents.month,dateComponents.day];
//        [self getDayAgencyMposDetail];
//    }else if(datePicker.datePickerMode == PGDatePickerModeYearAndMonth){
//        self.monthStr = [NSString stringWithFormat:@"%ld-%ld",dateComponents.year,dateComponents.month];
//        [self getMonthAgencyMposDetail];
//    }
//
//
