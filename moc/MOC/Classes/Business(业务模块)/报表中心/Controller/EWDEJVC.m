...
@interface EWDEJVC ()<UITableViewDelegate,UITableViewDataSource,PGDatePickerDelegate>
...
-(void)getDayAgencyTraditionalPosDetail{
    [self getDayMerchantTraditionalPosDetail:@{@"date":[self.dayStr stringByReplacingOccurrencesOfString:@"-" withString:@""],@"pos_type":@"epos"} completion:^(id object, NSString *error) {
        if (object) {
            self.dayData = object;
...
-(void)getMonthAgencyTraditionalPosDetail{
    [self getMonthMerchantTraditionalPosDetail:@{@"date":[self.monthStr stringByReplacingOccurrencesOfString:@"-" withString:@""],@"pos_type":@"epos"} completion:^(id object, NSString *error) {
        if (object) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    view.chartBlock  = ^(id data) {
        if (indexPath.section == 0) {
            [MXRouter openURL:@"lcwl://PerformanceChartVC" parameters:@{@"type":@"EWD",@"subType":@"month",@"dateStr":[self.monthStr stringByReplacingOccurrencesOfString:@"-" withString:@""]}];
        }else{
            [MXRouter openURL:@"lcwl://PerformanceChartVC" parameters:@{@"type":@"EWD",@"subType":@"day",@"dateStr":[self.dayStr stringByReplacingOccurrencesOfString:@"-" withString:@""]}];
        }
    };
