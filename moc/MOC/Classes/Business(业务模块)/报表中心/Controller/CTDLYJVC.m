...
@interface CTDLYJVC ()<UITableViewDelegate,UITableViewDataSource,PGDatePickerDelegate>
...
-(void)getDayAgencyTraditionalPosDetail{
    [self getDayAgencyTraditionalPosDetail:@{@"date":[self.dayStr stringByReplacingOccurrencesOfString:@"-" withString:@""]} completion:^(id object, NSString *error) {
        if (object) {
            self.dayData = object;
...
-(void)getMonthAgencyTraditionalPosDetail{
    [self getMonthAgencyTraditionalPosDetail:@{@"date":[self.monthStr stringByReplacingOccurrencesOfString:@"-" withString:@""]} completion:^(id object, NSString *error) {
        if (object) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    view.chartBlock  = ^(id data) {
         if (indexPath.section == 0) {
             [MXRouter openURL:@"lcwl://PerformanceChartVC" parameters:@{@"type":@"CTDL",@"subType":@"month",@"dateStr":[self.monthStr stringByReplacingOccurrencesOfString:@"-" withString:@""]}];
         }else{
             [MXRouter openURL:@"lcwl://PerformanceChartVC" parameters:@{@"type":@"CTDL",@"subType":@"day",@"dateStr":[self.dayStr stringByReplacingOccurrencesOfString:@"-" withString:@""]}];
         }
    };
...
- (void)datePicker:(PGDatePicker *)datePicker didSelectDate:(NSDateComponents *)dateComponents{
    ...
    if (datePicker.datePickerMode == PGDatePickerModeDate) {
        self.dayStr = [NSDate dateString:date format:@"yyyy-MM-dd"];
        NSCalendar *calendar = [NSCalendar currentCalendar];
        NSDate* date = [calendar dateFromComponents:dateComponents];
        self.dayStr = [NSDate dateString:date format:@"yyyy-MM-dd"];
        ...
        self.monthStr = [NSDate dateString:date format:@"yyyy-MM"];
        NSCalendar *calendar = [NSCalendar currentCalendar];
        NSDate* date = [calendar dateFromComponents:dateComponents];
        self.monthStr = [NSDate dateString:date format:@"yyyy-MM"];
