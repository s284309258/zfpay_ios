...
@interface MWDEJVC ()<UITableViewDelegate,UITableViewDataSource,PGDatePickerDelegate>
...
-(void)initData{
    ...
    [self getDayMerchantMposDetail];
    [self getMonthMerchantMposDetail];
    
    self.headerData = @[@{@"color":@"#01C088",@"text":@"月业绩",@"desc":@"2019-08"},
                        @{@"color":@"#01C088",@"text":@"日业绩",@"desc":@"2019-08-06"}];
}

-(void)getDayMerchantMposDetail{
    [self getDayMerchantMposDetail:@{@"date":[self.dayStr stringByReplacingOccurrencesOfString:@"-" withString:@""]} completion:^(id object, NSString *error) {
        if (object) {
            self.dayData = object;
...
-(void)getMonthMerchantMposDetail{
    [self getMonthMerchantMposDetail:@{@"date":[self.monthStr stringByReplacingOccurrencesOfString:@"-" withString:@""]} completion:^(id object, NSString *error) {
        if (object) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    view.chartBlock  = ^(id data) {
        if (indexPath.section == 0) {
            [MXRouter openURL:@"lcwl://PerformanceChartVC" parameters:@{@"type":@"MWD",@"subType":@"month",@"dateStr":[self.monthStr stringByReplacingOccurrencesOfString:@"-" withString:@""]}];
        }else{
            [MXRouter openURL:@"lcwl://PerformanceChartVC" parameters:@{@"type":@"MWD",@"subType":@"day",@"dateStr":[self.dayStr stringByReplacingOccurrencesOfString:@"-" withString:@""]}];
        }
    };
